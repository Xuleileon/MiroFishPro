...
RUN apt-get update && apt-get install -y --no-install-recommends nodejs npm build-essential libmagic1 curl && rm -rf /var/lib/apt/lists/*
...
RUN cd backend && uv sync --frozen \
    && uv pip install --python .venv torch --index-url https://download.pytorch.org/whl/cpu --reinstall --no-cache-dir \
